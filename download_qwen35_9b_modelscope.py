#!/usr/bin/env python3
"""
从魔搭 ModelScope 拉取 Qwen/Qwen3.5-9B（与网页一致）:
https://www.modelscope.cn/models/Qwen/Qwen3.5-9B

用法:
  pip install modelscope
  python download_qwen35_9b_modelscope.py
  python download_qwen35_9b_modelscope.py --local-dir /path/to/save
...
_DEFAULT_LOCAL_DIR = os.path.join(_AUTODL_TMP, "Qwen3.5-9B")
_MODELSCOPE_PAGE = "https://www.modelscope.cn/models/Qwen/Qwen3.5-9B"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Download Qwen/Qwen3.5-9B from ModelScope (魔搭)."
    )
    p.add_argument(
        "--model-id",
        default="Qwen/Qwen3.5-9B",
        help="ModelScope 模型 ID（默认与网页一致: Qwen/Qwen3.5-9B）",
    )
    p.add_argument(
        "--local-dir",
